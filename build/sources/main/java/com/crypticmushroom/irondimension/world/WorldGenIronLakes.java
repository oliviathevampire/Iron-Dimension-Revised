...
public class WorldGenIronLakes extends WorldGenerator {
    private final Block block;

    public WorldGenIronLakes(Block blockIn) {
        this.block = blockIn;
    }
                ...
                for (int j3 = 0; j3 < 16; ++j3) {
                    for (int j4 = 4; j4 < 8; ++j4) {
                        if (aboolean[(i2 * 16 + j3) * 8 + j4]) {
                            ...
                            if (worldIn.getBlockState(blockpos).getBlock() == BlocksIDL.iron_dirt && worldIn.getLightFor(EnumSkyBlock.SKY, position.add(i2, j4, j3)) > 0) {

                                worldIn.setBlockState(blockpos, BlocksIDL.iron_grass.getDefaultState(), 2);
                            }
                        }
                    }
                }
